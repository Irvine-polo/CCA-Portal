...
<form id='form_response'>
	...
	$exists = $conn->query("SELECT * FROM response_completed WHERE eval_set = '$eval_set' AND username = '$referenceNumber'");
		...
		if (isset($_SESSION['vpaa'])) {
			$sql =$conn->query("SELECT * FROM users WHERE role = 'faculty' AND username <> '$referenceNumber' AND institute = 'IEAS' OR institute = 'IBM' OR institute = 'ICSLIS' order by institute");
		}else{
