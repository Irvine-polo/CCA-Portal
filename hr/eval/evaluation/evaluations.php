...
if ($completed == $total_table) {
	$date_completed = $conn ->query("SELECT * FROM evaluation_set WHERE title = '$eval_set' AND status = 'completed' order by date_completed ASC LIMIT 1")->fetch_assoc();
	echo"
	<div>
		<p ><strong>Status: </strong><i class='fa-solid fa-circle-check text-success'></i><span class='text-success'> Completed</span>  |  <strong>Date Completed: </strong> ".date("M d, Y",strtotime($date_completed['date_completed']))."</p>
	</div>";
}
else{
	echo"
