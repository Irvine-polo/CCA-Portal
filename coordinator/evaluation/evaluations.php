		...
		<select class="form-select me-2" id="faculty">
			...
			<?php 
				...
				while ($row = $sql->fetch_assoc()) {
					$id_e = $row['id'];
					$res_comp = $conn->query("SELECT DISTINCT faculty_id,eval_set,lastname,firstname FROM response_completed LEFT JOIN users ON response_completed.faculty_id = users.username WHERE eval_set = '$id_e' AND institute = '$institute' order by faculty_id ASC");
				}
