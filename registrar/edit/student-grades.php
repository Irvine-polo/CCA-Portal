...
<div class="d-flex justify-content-between align-items-center d-print-none mb-3">
	<h1 class="h3 mb-0">Edit Student Grades</h1>
					...
					if (mysqli_num_rows($result) > 0) {
						while ($row = mysqli_fetch_assoc($result)) {
							...
							echo "<tr class='text-center'>
								<td>" . $counter . "</td>
								<td class='student-number'>" . $row["student_number"] . "</td>
								...
								if (mysqli_num_rows($result2) > 0) {
									while ($row2 = mysqli_fetch_assoc($result2)) {
										echo "<td class='equivalent'>" . $row2["final_grade"] . "</td>
										<td class='remarks'>" . $row2["remarks"] . "</td>
										<td>
											<a class='btn btn-info btn-sm' href='./student-grade?academicYear=" . $academicYear . "&semester=" . $semester . "&classCode=" . $classCode . "&studentNumber=" . $studentNumber . "'>
												<i class='fa-solid fa-pen-to-square'></i>
											</a>
										</td>";
									}
								} else {
									echo "<td class='equivalent'></td>
									<td class='remarks'></td>
									<td>
										<a class='btn btn-info btn-sm' href='./student-grade?academicYear=" . $academicYear . "&semester=" . $semester . "&classCode=" . $classCode . "&studentNumber=" . $studentNumber . "'>
											<i class='fa-solid fa-pen-to-square'></i>
