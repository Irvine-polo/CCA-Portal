...
<div class="d-flex justify-content-between align-items-center d-print-none mb-3">
	<h1 class="h3 mb-0">Encode Final Grade</h1>

	<a class="btn btn-primary d-flex justify-content-between align-items-center" href="../view/student-grades?academicYear=<?= sanitize($_GET["academicYear"]); ?>&semester=<?= sanitize($_GET["semester"]); ?>">
...
<div class="alert alert-info" role="alert">
    <strong>Note!</strong> Press enter key to save the grade and move to the next student.
</div>
		...
		$(document).ready( 
			function() {
				$('#datatable').DataTable({
					...
					"serverSide": true,
					"ajax":{
						"url": `../../assets/api/faculty/finalized-grade.inc.php?getFinalizedGrades&academicYear=${academicYear}&semester=${semester}&classCode=${classCode}`,
						"type": "POST"
					},
						...
						$(".equivalent input").keypress(function(event) {
							if (event.keyCode == 13) {
								...
								if (allowedEquivalents.includes(finalGrade)) {
									$.ajax({
										url: "../../assets/includes/faculty/finalized-grade.inc.php",
										type: "POST",
										data: {
											updateStudentGrade : "",
