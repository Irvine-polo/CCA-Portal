...
if (isset($_GET["recoverUser"])) {
	...
	if (!mysqli_query($conn, $sql)) {
		header("Location: " . $baseUrl . "secretary/?error=Recover User error&username=" . $username . "&initialPassword=" . $initialPassword);
		exit();
	}

	header("Location: " . $baseUrl . "secretary/recover/user?success=Recovered User successfully&username=" . $username . "&initialPassword=" . $initialPassword);
	exit();
}
...
if (isset($_GET["disableUser"])) {
	...
	if (!mysqli_query($conn, $sql)) {
		header("Location: " . $baseUrl . "secretary/users?error=Disable User error");
		exit();
	}

	header("Location: " . $baseUrl . "secretary/?success=Disabled User successfully");
	exit();
}
...
if (isset($_GET["enableUser"])) {
	...
	if (!mysqli_query($conn, $sql)) {
		header("Location: " . $baseUrl . "secretary/users?error=Enable User error");
		exit();
	}

	header("Location: " . $baseUrl . "secretary/?success=Enabled User successfully");
	exit();
}
...
if (isset($_POST["submitEditUser"])) {
    ...
    if (!mysqli_query($conn, $sql)) {
    	header("Location: " . $baseUrl . "secretary/edit/user?userId=" . $userId . "&error=Upadate <b>USER</b> error");
    	exit();
    }
    
    header("Location: " . $baseUrl . "secretary/edit/user?userId=" . $userId . "&success=Upadated <b>USER</b> successfully");
	exit();
}