...
if (isset($_POST["submitUpdateProfile"])) {
	...
	if (!empty($_FILES['avatar']['tmp_name'])) {
		...
		if (!mysqli_query($conn, $sql)) {
			header("Location: " . $baseUrl . "hr/profile/?error=Update Profile error");
			exit();
		}		
	} else {
	...
	header("Location: " . $baseUrl . "hr/profile/?success=Updated profile successfully");
	exit();
}
...
if (isset($_POST["submitChangePassword"])) {
	...
	if (mysqli_num_rows($result) > 0) {
		while ($row = mysqli_fetch_assoc($result)) {
			if (!password_verify($currentPassword, $row["password"])) {
				header("Location: " . $baseUrl . "hr/profile/change-password?error=Incorrect Current Password");
				exit();
			}
		}
	}
	...
	if ($newPassword != $confirmNewPassword) {
		header("Location: " . $baseUrl . "hr/profile/change-password?error=Passwords mismatch");
		exit();
	}
	...
	if (!mysqli_query($conn, $sql)) {
		header("Location: " . $baseUrl . "hr/profile/change-password?error=Change Password error");
		exit();
	}

	header("Location: " . $baseUrl . "hr/profile/change-password?success=Changed Password successfully");
	exit();
}