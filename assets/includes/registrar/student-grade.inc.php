...
if (isset($_POST["updateStudentGrade"])) {
	...
	if (mysqli_num_rows($result) > 0) {
		while ($row = mysqli_fetch_assoc($result)) {
			if (!password_verify($password, $row["password"])) {
				header("Location: ../../../registrar/edit/student-grade?academicYear=" . $academicYear . "&semester=" . $semester . "&classCode=" . $classCode . "&studentNumber=" . $studentNumber . "&error=Incorrect Password");
				exit();
			}
		}
	}
	...
	if (mysqli_num_rows($result) > 0) {
		...
		if (!mysqli_query($conn, $sql2)) {
			header("Location: ../../../registrar/edit/student-grade?academicYear=" . $academicYear . "&semester=" . $semester . "&classCode=" . $classCode . "&studentNumber=" . $studentNumber . "&error=Update <b>STUDENT GRADE</b> error");
			exit();
		}
	} else {
		// INSERT
		...
		if (!mysqli_query($conn, $sql2)) {
			header("Location: ../../../registrar/edit/student-grade?academicYear=" . $academicYear . "&semester=" . $semester . "&classCode=" . $classCode . "&studentNumber=" . $studentNumber . "&error=Insert <b>STUDENT GRADE</b> error");
			exit();
		}
	}
	...
	if (mysqli_num_rows($result) > 0) {
		while ($row = mysqli_fetch_assoc($result)) {
			...
			if (!mysqli_query($conn, $sql)) {
				header("Location: ../../../registrar/edit/student-grade?academicYear=" . $academicYear . "&semester=" . $semester . "&classCode=" . $classCode . "&studentNumber=" . $studentNumber . "&error=Insert <b>NOTIFICATION</b> error");
				exit();
			}
		}
	}			

	header("Location: ../../../registrar/edit/student-grade?academicYear=" . $academicYear . "&semester=" . $semester . "&classCode=" . $classCode . "&studentNumber=" . $studentNumber . "&success=Updated <b>STUDENT GRADE</b> successfully");
	exit();
}