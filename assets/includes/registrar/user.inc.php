...
if (isset($_GET["recoverUser"])) {
	...
	if (!mysqli_query($conn, $sql)) {
		header("Location: " . $baseUrl . "registrar/users?error=Recover User error&username=" . $username . "&initialPassword=" . $initialPassword);
		exit();
	}

	header("Location: " . $baseUrl . "registrar/recover/user?success=Recovered User successfully&username=" . $username . "&initialPassword=" . $initialPassword);
	exit();
}
...
if (isset($_GET["disableUser"])) {
	...
	if (!mysqli_query($conn, $sql)) {
		header("Location: " . $baseUrl . "registrar/users?error=Disable User error");
		exit();
	}

	header("Location: " . $baseUrl . "registrar/users?success=Disabled User successfully");
	exit();
}
...
if (isset($_GET["enableUser"])) {
	...
	if (!mysqli_query($conn, $sql)) {
		header("Location: " . $baseUrl . "registrar/users?error=Enable User error");
		exit();
	}

	header("Location: " . $baseUrl . "registrar/users?success=Enabled User successfully");
	exit();
}
...
if (isset($_POST["submitImportUsers"])) {
	...
	while ($row = fgetcsv($handle)) {
		...
		$institute = "";
		$emailAddress = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', sanitize($row[5]));
		$username = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', sanitize($row[0]));
		$initialPassword = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', sanitize($row[4]));
		$password = "";
		$role = "student";
		$isActive = 1;
...
if (isset($_POST["submitEditUser"])) {
    ...
    if (!mysqli_query($conn, $sql)) {
    	header("Location: " . $baseUrl . "registrar/edit/user?userId=" . $userId . "&error=Upadate <b>USER</b> error");
    	exit();
    }
    
    header("Location: " . $baseUrl . "registrar/edit/user?userId=" . $userId . "&success=Upadated <b>USER</b> successfully");
	exit();
}