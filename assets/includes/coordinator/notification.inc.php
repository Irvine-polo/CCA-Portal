...
if (isset($_GET["viewNotification"])) {
	...
	if (!mysqli_query($conn, $sql)) {
		header("Location: " . $baseUrl . "coordinator/view/notification?notificationId=" . $notificationId . "error=Update <b>NOTIFICATION STATUS</b> error");
		exit();
	}

	header("Location: " . $baseUrl . "coordinator/view/notification?notificationId=" . $notificationId);
	exit();
}
...
if (isset($_POST["loadMoreNotifications"])) {
	...
	if (mysqli_num_rows($result) > 0) {
		while ($row = mysqli_fetch_assoc($result)) {
			if ($row["has_seen"] == 0) {
				echo "<a href='../../assets/includes/coordinator/notification.inc.php?viewNotification&notificationId=" . $row["id"] . "' class='list-group-item fw-bold'>
				...
				echo "<a href='../../assets/includes/coordinator/notification.inc.php?viewNotification&notificationId=" . $row["id"] . "' class='list-group-item'>
					<div class='row g-0 align-items-center'>
						<div class='col-10'>
							<div class='text-dark'>" . $row["title"] . "</div>
							<div class='text-muted small mt-1'>" . $row["description"] . "</div>
