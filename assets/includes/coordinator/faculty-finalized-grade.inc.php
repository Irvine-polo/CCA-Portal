...
if (isset($_POST["submitImportFinalizedGrades"])) {
	...
	while ($row = fgetcsv($handle)) {
		...
		if (mysqli_num_rows($result) > 0) {
			...
			if (!mysqli_query($conn, $sql2)) {
				$data["type"] = "error";
				$data["value"] = "Update <b>FINALIZED GRADES</b> error";
			...
			if (!mysqli_query($conn, $sql2)) {
				$data["type"] = "error";
				$data["value"] = "Import <b>FINALIZED GRADES</b> error";
	...
	$data["type"] = "success";
	$data["value"] = "Imported <b>FINALIZED GRADES</b> successfully";
...
if (isset($_GET["submitfinalizeGrades"])) {
	...
	if (mysqli_num_rows($result) > 0) {
		while ($row = mysqli_fetch_assoc($result)) {
			...
			if (mysqli_num_rows($result2) == 0) {
				header("Location: " . $baseUrl . "coordinator/view/faculty-student-grades?academicYear=" . $academicYear . "&semester=" . $semester . "&error=Incomplete <b>STUDENT GRADES</b> error");
				exit();	
			}
		}
	}
	...
	if (!mysqli_query($conn, $sql)) {
		header("Location: " . $baseUrl . "coordinator/view/faculty-student-grades?academicYear=" . $academicYear . "&semester=" . $semester . "&error=Update <b>CLASS SCHEDULE</b> status error");
		exit();
	}

	header("Location: " . $baseUrl . "coordinator/view/faculty-student-grades?academicYear=" . $academicYear . "&semester=" . $semester . "&success=Finalized <b>STUDENT GRADES</b> successfully");
	exit();
}