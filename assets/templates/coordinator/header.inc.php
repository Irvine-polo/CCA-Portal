	...
	elseif ($data['start_date'] <= $current_date && $data['status'] == 'pending' ) {
		...
		if ($exists == 0) {
			...
			while($row = $call_faculty_sql->fetch_assoc()){
				...
				$title = $title_notif;
				$description = "<b>".$title."</b><p> is now available. Go to Evaluate Faculty tab and click evaluate. This Evaluation Ends on ".$end_date."</p>";
		...
		if ($current_date2 >= $data2['start_share'] ) {
			...
			if ($exists == 0) {
				...
				while($row = $call_faculty_sql->fetch_assoc()){
					$username = $row['username'];
					$title = $title_notif;
					$description = "<b>".$title."</b><p> Go to Evaluation Results tab to view results.</p>";
					$createdAt = date("Y-m-d H:i:s", time());
					$hasSeen = 0;
					$insert = $conn->query("INSERT INTO notifications(username, title, description, created_at, has_seen)VALUES('$username','$title','$description','$createdAt','$hasSeen')");
					}
	...
	<div class="wrapper">
		<nav class="sidebar js-sidebar user-select-none d-print-none" id="sidebar">
			<div class="sidebar-content js-simplebar">
				<a class="sidebar-brand d-flex flex-column align-items-center text-center" href="<?= $baseUrl; ?>coordinator">
				...
				<ul class="sidebar-nav">
					<li class="sidebar-header">Faculty</li>

					<li class="sidebar-item <?= page("facultyClassSchedules", $page); ?>">
						<a class="sidebar-link" href="<?= $baseUrl; ?>coordinator">
							<i class="fa-solid fa-calendar-week align-middle"></i>
							<span class="align-middle">Class Schedules</span>
						</a>
					</li>

					<li class="sidebar-item <?= page("facultyStudentGrades", $page); ?>">
						<a class="sidebar-link" href="<?= $baseUrl; ?>coordinator/faculty-student-grades">
					...
					<li class="sidebar-item <?= page("evaluation-tab", $page); ?>">
						<a class="sidebar-link" href="<?= $baseUrl; ?>coordinator/evaluation-tab">
							<i class="fa-solid fa-chart-pie align-middle"></i>
							<span class="align-middle">Evaluate Faculty</span>
						</a>
					</li>

					<li class="sidebar-header">Coordinator</li>

					<li class="sidebar-item <?= page("evaluations", $page); ?>">
						<a class="sidebar-link" href="<?= $baseUrl; ?>coordinator/evaluation">
							<i class="fa fa-area-chart"></i>
							<span class="align-middle">Evaluation Report</span>
						</a>
					</li>
					...
					<ul class="navbar-nav navbar-align">
						<li class="nav-item dropdown">
							...
							<div class="dropdown-menu dropdown-menu-lg dropdown-menu-end py-0" aria-labelledby="alertsDropdown">
								...
								<div class="list-group">
									...
									if (mysqli_num_rows($result) > 0) {
										while ($row = mysqli_fetch_assoc($result)) {
											if ($row["has_seen"] == 0) {
												echo "<a href='" . $baseUrl . "assets/includes/coordinator/notification.inc.php?viewNotification&notificationId=" . $row["id"] . "' class='list-group-item fw-bold'>
												...
												echo "<a href='" . $baseUrl . "assets/includes/coordinator/notification.inc.php?viewNotification&notificationId=" . $row["id"] . "' class='list-group-item'>
													<div class='row g-0 align-items-center'>
														<div class='col-2'>
															<i class='text-primary' data-feather='bell'></i>
														</div>
								...
								<div class="dropdown-menu-footer">
									<a href="<?= $baseUrl; ?>coordinator/view/notifications" class="text-muted">Show all notifications</a>
								</div>
							</div>
						</li>
						...
						<li class="nav-item dropdown">
							...
							<div class="dropdown-menu dropdown-menu-end">
								<a class="dropdown-item" href="<?= $baseUrl; ?>coordinator/profile">
