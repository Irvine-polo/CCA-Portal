...
if (isset($_GET["getAcademicYears"])) {
	...
	if (mysqli_num_rows($result) > 0) {
		while ($row = mysqli_fetch_assoc($result)) {
			...
			$subdata[] =	"<button type='button' class='btn btn-success btn-sm' data-bs-toggle='modal' data-bs-target='#activateModal' data-bs-name='A.Y " . $row["academic_year"] . ", " . $row["semester"] . " Semester' data-bs-href='../assets/includes/admin/academic-year.inc.php?activateAcademicYear&id=" . $row["id"] . "' title='activate'>
