...
if (isset($_GET["getClassRecords"])) {
	...
	if (!empty($request["search"]["value"])) {
		$sql .= "AND (dean_name LIKE '%" . $request["search"]["value"] . "%' ";
		...
		while ($row = mysqli_fetch_assoc($result)) {
			...
			if ($row["status"] == "finalized") {
				...
				$subdata[] = "<div class='btn-group'>
					...
					<button type='button' class='btn btn-main btn-sm disabled' data-bs-toggle='modal' data-bs-target='#finalizeModal' data-bs-name='" . $row["class_code"] . "' data-bs-href='../../assets/includes/dean/faculty-finalized-grade.inc.php?submitfinalizeGrades&academicYear=" . $academicYear . "&semester=" . $semester . "&classCode=" . $row["class_code"] . "' title='finalize'>
				...
				$subdata[] = "<div class='btn-group'>
					...
					<button type='button' class='btn btn-main btn-sm' data-bs-toggle='modal' data-bs-target='#finalizeModal' data-bs-name='" . $row["class_code"] . "' data-bs-href='../../assets/includes/dean/faculty-finalized-grade.inc.php?submitfinalizeGrades&academicYear=" . $academicYear . "&semester=" . $semester . "&classCode=" . $row["class_code"] . "' title='finalize'>
						<i class='fa-solid fa-flag'></i>
					</button>
				</div>";
			}
