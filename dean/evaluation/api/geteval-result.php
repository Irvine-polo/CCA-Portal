...
if (isset($_GET['eval_type'])) {
	...
	while ($row = $sql->fetch_assoc()) {
		$id_e = $row['id'];
		$res_comp = $conn->query("SELECT DISTINCT faculty_id,eval_set FROM response_completed LEFT JOIN users ON response_completed.faculty_id = users.username WHERE eval_set = '$id_e' AND institute = '$institute' order by faculty_id ASC");
		
	}
	
}
//FACULTY FILTER
$header_title = 'FACULTY PERFORMANCE EVALUATION';
if (isset($_GET['faculty'])) {
		$title = $_GET['title'];
		$faculty = $_GET['faculty'];
		$sql = $conn->query("SELECT * FROM evaluation_set WHERE title = '$title'");
		$header_title = "SUMMARY OF FACULTY EVALUATION";
		while ($row = $sql->fetch_assoc()) {
			$id_e = $row['id'];
			$res_comp = $conn->query("SELECT DISTINCT faculty_id,eval_set FROM response_completed LEFT JOIN users ON response_completed.faculty_id = users.username WHERE eval_set = '$id_e'  AND faculty_id = '$faculty'");
...
//INSTITUTE FILTER
if (isset($_GET['filter_institute'])) {
	$title = $_GET['title'];
	$filter_institute = $_GET['filter_institute'];
	$sql = $conn->query("SELECT * FROM evaluation_set WHERE title = '$title'");
	while ($row = $sql->fetch_assoc()) {
			$id_e = $row['id'];
			$res_comp = $conn->query("SELECT DISTINCT faculty_id,eval_set FROM response_completed LEFT JOIN users ON response_completed.faculty_id = users.username WHERE eval_set = '$id_e'  AND institute = '$filter_institute'");
		
	}
	if ($_GET['filter_institute'] == 'All') {

			$sql = $conn->query("SELECT * FROM evaluation_set WHERE title = '$title'");
			while ($row = $sql->fetch_assoc()) {
				$id_e = $row['id'];
				$res_comp = $conn->query("SELECT DISTINCT faculty_id,eval_set FROM response_completed LEFT JOIN users ON response_completed.faculty_id = users.username WHERE eval_set = '$id_e'  order by faculty_id ASC");
...
$output .= "<div class='text-center'>
				...
				<h5 class='fw-bold small mb-1'>".$header_title."</h5>
...
while ($faculname = $res_comp->fetch_assoc()) {
	...
	$a_dean = [];
	$a_coor = [];
	$evaluatin_sql =$conn->query("SELECT * FROM evaluation_set WHERE title = '$title' AND status = 'completed' order by order_by ASC");
